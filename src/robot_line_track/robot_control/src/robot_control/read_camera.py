...
def say_it_works():
    rospy.loginfo(
        "Success! You just have imported a Python module in another package.")
...
def laplacian_edge_detection_hsv(img):
    ...
    # L2gradient is a flag, which indicates whether a more accurate L2 norm is used to calculate the image gradient magnitude (cv2.L2gradient)
    edges = cv2.Laplacian(img, cv2.CV_64F)
    # cv2.imshow("img-laplacian-edge-detection-khsv", edges)
    return edges
...
def get_contours(grayimg, mod=cv2.RETR_LIST, method=cv2.CHAIN_APPROX_SIMPLE):

    # img, mode(cv2.RETR_LIST, cv2.RETR_EXTERNAL, cv2.RETR_TREE),method (cv2.CHAIN_APPROX_SIMPLE, cv2.CHAIN_APPROX_NONE)
    contours, hierarchy = cv2.findContours(
        grayimg, mod, method)  # img, mode(cv2.RETR_LIST, cv2.RETR_EXTERNAL, cv2.RETR_TREE),method (cv2.CHAIN_APPROX_SIMPLE, cv2.CHAIN_APPROX_NONE)
    # img, contours, contourIdx, color, thickness
    cv2.drawContours(grayimg, contours, -1, (0, 255, 0),
                     2)  # -1 means all contours
    # cv2.imshow("img-contours", grayimg)
