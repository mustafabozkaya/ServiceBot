...
class LineTrack():
    def __init__(self):
        ...
        rospy.Subscriber(self.camera_topic,
                         Image, self.cameraCallback)

        self.cmd_topic = "/cmd_vel"
        self.pub = rospy.Publisher(self.cmd_topic, Twist, queue_size=2)
        self.pub_image = rospy.Publisher('result_img', Image, queue_size=10)
        # self.createTackerbar()

    def createTackerbar(self):
        cv2.namedWindow("HSV trackbar", cv2.WINDOW_AUTOSIZE)
        cv2.createTrackbar("low_h", "HSV trackbar", 7, 179, self.nothing)
        cv2.createTrackbar("low_s", "HSV trackbar", 71, 255, self.nothing)
        cv2.createTrackbar("low_v", "HSV trackbar", 0, 255, self.nothing)
        cv2.createTrackbar("high_h", "HSV trackbar", 39, 179, self.nothing)
        cv2.createTrackbar("high_s", "HSV trackbar", 255, 255, self.nothing)
        cv2.createTrackbar("high_v", "HSV trackbar", 255, 255, self.nothing)
        cv2.createTrackbar("point1_x", "HSV trackbar", 640, 640, self.nothing)
        cv2.createTrackbar("point1_y", "HSV trackbar", 200, 480, self.nothing)
        cv2.createTrackbar("point2_x", "HSV trackbar", 60, 640, self.nothing)
        cv2.createTrackbar("point2_y", "HSV trackbar", 200, 480, self.nothing)
    ...
    def getTrackbarPos(self):
        ...
        return low_h, low_s, low_v, high_h, high_s, high_v,  point1_x, point1_y, point2_x, point2_y, point3_x, point3_y, point4_x, point4_y, camera_w_center
    ...
    def cameraCallback(self, mesaj):
        ...
        # 18.10.2022
        low_h = 7  # (Düşük ışık değerlerine göre günceleme yapıldı.)
        low_s = 71
        low_v = 0
        high_h = 39  # ( Daha belirgin aralık için üst limit azaltıldı.)
        high_s = 255
        high_v = 255
        point1_x = 600
        point1_y = 200  # (Üst görüş açısı kısaltıldı.)
        point2_x = 63
        point2_y = 200  # (Üst görüş açısı kısaltıldı.)
        point3_x = 0
        point3_y = 480
        point4_x = 640
        point4_y = 480
        camera_w_center = 340  # ( Kamera orta noktası güncellendi.)
        ...
        # 05.10.2022 Eski şerit değerleri
        # low_h = 18
        # low_s = 71
        # low_v = 0
        # high_h = 58
    ...
    def line_track(self, low_h, low_s, low_v, high_h,
                   high_s, high_v, point1_x, point1_y,
                   point2_x, point2_y, point3_x, point3_y,
                   point4_x, point4_y, camera_w_center, img_resized):
        ...
        if len(contours) > 0:
            ...
            if M['m00'] > 75:  # if m00 is not zero,m00 means the area of the image
                ...
                self.cmd_vel.angular.z = round(-error /
                                               600*angular_corefficient, 2)
            ...
            img_list = ([mask_range, adaptive_threshold],
                        [mask_blur, result_bitwise])

            stack_img = utl.stackImageslabels(img_list, img_labels, 0.5)
            # cv2.imshow("stack", stack_img)
            self.pub_image.publish(
                self.bridge.cv2_to_imgmsg(stack_img, "bgr8"))
