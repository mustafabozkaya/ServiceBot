...
class LineTrackCamera():
    ...
    def createTackerbar(self):
        ...
        cv2.createTrackbar("low_v", "HSV trackbar", 69, 255, self.nothing)
        cv2.createTrackbar("high_h", "HSV trackbar", 150, 179, self.nothing)
        cv2.createTrackbar("high_s", "HSV trackbar", 154, 255, self.nothing)
        cv2.createTrackbar("high_v", "HSV trackbar", 150, 255, self.nothing)
    ...
    def line_track2(self, low_h, low_s, low_v, high_h, high_s, high_v, img_resized):
        ...
        for idx in sortIdx:
            ...
            # find the contours in the mask
        cnts, h = read_camera.get_contours(mask)
    ...
    def line_track3(self, low_h, low_s, low_v, high_h, high_s, high_v, img_resized):
        ...
        if len(contours) > 0:
            ...
            stack_img = utl.stackImageslabels(img_list, img_labels, 0.6)
            cv2.imshow("stack", stack_img)
            cv2.waitKey(1)  # wait for 1 ms before moving to the next frame

    def line_track4(self, low_h, low_s, low_v, high_h, high_s, high_v, img_resized):
        ...
        if len(contours) > 0:
            ...
            if M['m00'] > 200:  # if m00 is not zero,m00 means the area of the image
                ...
                self.cmd_vel.linear.x = 1 * linear_corefficient
                # set the angular velocity to the distance divided by 100
                self.cmd_vel.angular.z = -error/900*angular_corefficient
                self.pub.publish(self.cmd_vel)
            else:
                pass
                self.cmd_vel.linear.x = 0.0
